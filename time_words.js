...
function numberToWord(num) {
  if (num < 1 || num > 59 || num % 1 != 0) {
    throw new Error(
      "Argument for numberToWord must be an integer between 1 and 59, inclusive."
    );
  }
...
function timeToWords(hours, minutes) {
  if (hours < 0 || hours > 24 || hours % 1 != 0) {
    throw new Error(
      "Hours argument for timeToWord must be an integer between 0 and 24, inclusive."
    );
  }

  if (minutes < 0 || minutes > 59 || minutes % 1 != 0) {
    throw new Error(
      "Minutes argument for timeToWord must be an integer between 0 and 60, inclusive."
    );
  }
  ...
  if (minutes <= 30) {
    if (minutes in SPECIAL_NUMBER_WORDS) {
      ...
      return `${hoursWord} ${minutes < 10 ? "oh " : ""}${numberToWord(
        minutes
      )}`;
    }
  } else {
    ...
    if (remaining in SPECIAL_NUMBER_WORDS) {
      ...
      return `${numberToWord(remaining)} minute${
        remaining == 1 ? "" : "s"
      } to ${nextHourWord}`;
    }
  }
